...
const slugs = [
  'ziwei-hongluan-tianxi', 'ziwei-xianchi-tianyao', 'ziwei-tianma-xing',
  'ziwei-tianxing-xing', 'ziwei-jieshen-xing', 'ziwei-guchen-guasu',
  'ziwei-santai-bazuo', 'ziwei-huagai-xing', 'ziwei-longchi-fengge',
  'ziwei-tianku-tianxu'
];
...
for (const slug of slugs) {
  ...
  for (const [p, isEn] of [[cnPath, false], [enPath, true]]) {
    ...
    if (!html.includes(isEn ? 'articles/' + slug : 'articles/en/' + slug)) errors.push(`${slug} ${isEn?'EN':'CN'}: missing language switch link`);
