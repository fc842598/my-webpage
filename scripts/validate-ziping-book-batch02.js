...
function main() {
  const samples = readJson(path.join('tmp', 'tianji-book-validation-batch-02.json'));
  ...
  console.log('=== Book Validation Batch 02 Replay ===');
  ...
  if (liunianFailed.length) {
    console.log('\n=== Liunian Mismatches ===');
    liunianFailed.slice(0, 12).forEach(row => {
      console.log(`- ${row.id}: ${row.inputBirth} ${row.gender} actualLen=${row.actualLength} truthLen=${row.truthLength}`);
