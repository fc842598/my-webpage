...
for (const slug of slugs) {
  ...
  if (!fs.existsSync(cnPath)) { errors.push(`CN missing: ${slug}`); continue; }
  const cn = fs.readFileSync(cnPath, 'utf8');
  const enPath = path.join('articles', 'en', `${slug}.html`);
  if (!fs.existsSync(enPath)) { errors.push(`EN missing: ${slug}`); continue; }
  ...
  const checks = [
    ...
    ['in topic page', mainTopic.includes(slug) || auxTopic.includes(slug)],
    ['in CN feed', cnFeed.includes(slug)],
    ['in EN feed', enFeed.includes(slug)],
    ['in sitemap-articles', smA.includes(slug)],
...
else { console.log(`\n${errors.length} ERRORS:`); errors.slice(0,20).forEach(e => console.log(`  - ${e}`)); }
