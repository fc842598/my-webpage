...
const slugs = [
  'ziwei-guanlugong-huaquan',
  'ziwei-tianzhaigong-huaquan',
  'ziwei-fudegong-huaquan',
  'ziwei-caibogong-huaquan',
  'ziwei-fumugong-huaquan',
  'ziwei-minggong-huake',
  'ziwei-xiongdigong-huake',
  'ziwei-fuqigong-huake',
  'ziwei-zinvgong-huake',
  'ziwei-caibogong-huake'
];
...
for (const slug of slugs) {
  ...
  // JSON-LD parse
  const cnJsonBlocks = [...cn.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)];
  check(cnJsonBlocks.length === 2, slug + ' CN has 2 JSON-LD blocks');
  for (const m of cnJsonBlocks) {
    try { JSON.parse(m[1].trim()); } catch (e) { check(false, slug + ' CN JSON-LD parse: ' + e.message); }
  }
  ...
  const enJsonBlocks = [...en.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)];
  check(enJsonBlocks.length === 1, slug + ' EN has 1 JSON-LD block');
  for (const m of enJsonBlocks) {
    try { JSON.parse(m[1].trim()); } catch (e) { check(false, slug + ' EN JSON-LD parse: ' + e.message); }
  }
}

// Check indexes contain all slugs
...
// Check feed item counts
const cnFeedCount = (cnFeed.match(/<item>/g) || []).length;
const enFeedCount = (enFeed.match(/<item>/g) || []).length;
check(cnFeedCount === 80, 'CN feed has ' + cnFeedCount + ' items (expected 80)');
check(enFeedCount === 80, 'EN feed has ' + enFeedCount + ' items (expected 80)');

// Check JSON-LD in indexes
for (const [name, content] of [['CN index', cnIndex], ['EN index', enIndex], ['Topic', topic]]) {
  ...
  for (const m of blocks) {
    try { JSON.parse(m[1].trim()); } catch (e) { check(false, name + ' JSON-LD parse: ' + e.message); }
  }
}
