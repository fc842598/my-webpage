...
function sourceArchiveScreenV2(screen) {
  ...
  return `
    ...
    ${figBox("source-3-master-v2", 16, 282, 358, 108, "converted-card", "border-radius:18px;background:linear-gradient(110deg,#fffefb,#f8f2e6);box-shadow:0 10px 24px rgba(92,64,32,.11);")}
    ...
    ${figText("source-3-master-desc-v2", "\u5df2\u63a5\u5165\u547d\u76d8\uff0c\u53ef\u76f4\u63a5\u5f00\u542f\u5bf9\u8bdd", 112, 358, 208, 12, "#948676", 400, "left", "line-height:1.45;")}
    ${sourceAppBottomNav("\u9605\u5929AI", 755)}
  `;
}
...
function renderWentianChatContextSheet(chatContext) {
  ...
  if (type === "hepan") {
    ...
    rows.push(["模块", chatContext.tabTitle || ""]);
    rows.push(["卦象", chatContext.hexagramName || ""]);
    if (chatContext.liunianHeadline) rows.push(["流年", chatContext.liunianHeadline]);
    rows.push(["讲解", chatContext.masterTitle || ""]);
  }
  return `
...
function sourceAiChatScreen(screen) {
  ...
  const isYijingChat = chatContext?.type === "yijing";
  const isContextChat = isLiuyaoChat || isHepanChat || isLiurenChat || isYijingChat;
  ...
  const yijingFaqGroups = [
    {
      label: "卦象细断",
      items: [
        ["一句话断", "就这组易经推命，先用一句话断核心结果。"],
        ["关键象意", "这组卦最关键的象意是什么，落到我身上怎么理解？"],
        ["成败趋势", "按这组卦看，这件事更偏成还是偏阻？"],
        ["应期提醒", "这组卦能看出近期要留意的时间点吗？"]
      ]
    },
    {
      label: "行动建议",
      items: [
        ["现在怎么做", "结合命盘和这组卦，给我三步具体行动建议。"],
        ["该避什么", "这组卦里最需要避开的动作是什么？"],
        ["家宅六亲", "这组卦对家宅、父母或身边人有什么提醒？"],
        ["怎么化解", "如果卦象有阻，应该怎么调整和化解？"]
      ]
    }
  ];
...
const WENTIAN_FREE_DAILY_LIMIT = 20;
...
function sortWentianArchivesByDefault(archives, selectedId = "") {
  return (Array.isArray(archives) ? archives : [])
    ...
    .sort((left, right) => {
      const leftRank = left.archive?.id === selectedId ? 0 : 1;
      const rightRank = right.archive?.id === selectedId ? 0 : 1;
  ...
  try {
    ...
    if (saved && typeof saved === "object") {
      ...
      Object.assign(wentianChartAiState, {
        ...
        luckAiResults: saved.luckAiResults || {},
        luckDecadeKey: saved.luckDecadeKey || "",
        xiaoLianAgeKey: saved.xiaoLianAgeKey || "",
...
function saveWentianChartAiState() {
  try {
    ...
    all[getWentianChartAiStorageKey(wentianChartAiState.chartRecordId)] = {
      ...
      xiaoLianAgeKey: wentianChartAiState.xiaoLianAgeKey,
...
function getWentianLuckAiResultForSelected(selected = getWentianSelectedDecade(getWentianChartPayload())) {
  ...
  if (!key) return null;
  return wentianChartAiState.luckAiResults?.[key]
    || (key === selected.currentKey ? wentianChartAiState.results?.current_luck : null);
}
...
function renderWentianLuckReading(data, fallback, actionAttr, actionLabel) {
  ...
  const currentDecade = selected.items?.find((item) => item.key === selected.currentKey) || selected;
  ...
  return `
    <div class="wentian-mb-luck-hero">
      <span>十年大限</span>
      <h3>当前十年<br>现在是${escapeHtml(currentRangeLabel)}</h3>
    </div>
    ...
    <div class="wentian-mb-luck-list">
      ...
      `).join("") : `
        <section>
          ...
          ${renderWentianReadingParagraphs(selected.key === selected.currentKey
            ? `${selected.rangeLabel || "当前十年"} · ${selected.palaceName || "大限宫"}。当前十年会随“总批命”自动生成，也可以单独重批。`
            : `${selected.rangeLabel || "选中十年"} · ${selected.palaceName || "大限宫"}。点击“批选中十年”生成这一段解盘。`, fallback)}
        </section>
      `}
    </div>
...
function formatWentianGuaLine(lineType, lineNum) {
  const parts = [];
  if (lineNum) parts.push(`${lineNum}爻`);
  if (lineType) parts.push(lineType);
  return parts.join(" · ");
}

function getWentianLiunianGuaMap(chartData = {}) {
  if (typeof window === "undefined" || !window.ZipingRuntime) return {};
  const sizhu = chartData.sizhu || {};
  const birthYear = Number(chartData.birthYear || String(chartData.birthDate || chartData.solarTime || "").slice(0, 4));
  if (!birthYear || !sizhu.yearStem || !sizhu.monthStem || !sizhu.dayStem || !sizhu.hourStem) return {};
  try {
    return window.ZipingRuntime.compute(sizhu, chartData.gender || "male", birthYear)?.liunianMap || {};
  } catch (_err) {
    return {};
  }
}

function getWentianXiaoLianDomain(name = "") {
  const value = String(name || "");
  if (value.includes("迁移")) return "外部环境、出行、平台与机会";
  if (value.includes("命")) return "个人定位、选择与行动方式";
  if (value.includes("财帛")) return "现金流、资源配置与收益节奏";
  if (value.includes("官禄")) return "事业责任、职位与项目推进";
  if (value.includes("田宅")) return "家庭、房产、根基与安全感";
  if (value.includes("夫妻")) return "伴侣、合作与亲密关系";
  if (value.includes("子女")) return "作品、延伸成果与后辈事务";
  if (value.includes("疾厄")) return "身体消耗、压力与修复";
  if (value.includes("福德")) return "心态、休息与长期福分";
  if (value.includes("父母")) return "长辈、文书、规则与资质";
  if (value.includes("兄弟")) return "同辈、团队与资源分配";
  if (value.includes("仆役") || value.includes("交友")) return "人脉、客户与合作对象";
  return "当年应事落点";
}

function getWentianXiaoLianItems(chartData = {}) {
  const activeAge = Number(chartData.activeAge || chartData.realCurrentAge || 0);
  const currentYear = Number(chartData.currentYear || new Date().getFullYear());
  const liunianMap = getWentianLiunianGuaMap(chartData);
  const rows = Array.isArray(chartData.liunianTable) && chartData.liunianTable.length
    ? chartData.liunianTable
    : (getWentianAiCurrentYear(chartData).age ? [getWentianAiCurrentYear(chartData)] : []);
  return rows.map((row, index) => {
    const age = Number(row.age || row.virtualAge || 0);
    const solarYear = Number(row.solarYear || row.year || 0);
    const gua = normalizeWentianYijingResult(liunianMap[age] || liunianMap[String(age)] || null);
    const xiaolianBranch = row.xiaoLianBranch || row.xiaolianBranch || row.xiaoLian || "";
    ...
    const oppositeBranch = row.oppositeBranch || (oppositeIndex >= 0 ? WENTIAN_SHICHEN[(oppositeIndex + 6) % 12] : "");
    const oppositePalace = getWentianAiPalaceByBranch(chartData, oppositeBranch);
    const xiaolianPalaceName = row.xiaolianPalaceName || xiaolianPalace?.name || (xiaolianBranch ? `${xiaolianBranch}宫` : "");
    const oppositePalaceName = row.oppositePalaceName || oppositePalace?.name || (oppositeBranch ? `${oppositeBranch}宫` : "");
    const key = String(age || solarYear || index + 1);
    return {
      key,
      age,
      solarYear,
      yearGanzhi: row.yearGanzhi || "",
      xiaolianBranch,
      xiaolianPalaceName,
      xiaolianPalace: getWentianAiPalacePayload(xiaolianPalace || row.xiaolianPalace),
      oppositeBranch,
      oppositePalaceName,
      oppositePalace: getWentianAiPalacePayload(oppositePalace || row.oppositePalace),
      liunianGuaName: gua?.name || row.liunianGuaName || "流年小限",
      liunianGuaPeriod: gua?.period || row.liunianGuaPeriod || String(solarYear || ""),
      lineNum: row.lineNum || gua?.lineNum || gua?.line || "",
      lineType: row.lineType || gua?.lineType || gua?.lineText || "",
      domain: getWentianXiaoLianDomain(xiaolianPalaceName),
      isCurrent: !!((activeAge && age === activeAge) || (currentYear && solarYear === currentYear)),
    };
  }).filter((item) => item.key);
}

function resolveWentianSelectedXiaoLianYear(chartData = {}, selectedKey = "") {
  const items = getWentianXiaoLianItems(chartData);
  const current = items.find((item) => item.isCurrent) || items[0] || getWentianAiCurrentYear(chartData);
  const selected = items.find((item) => item.key === selectedKey) || current;
  return {
    ...selected,
    items,
    currentKey: current?.key || "",
...
function getWentianXiaoLianReadingGroups(data) {
  const sections = getWentianAiSections(data);
  const wanted = [
    { title: "年度触发", keys: ["触发", "小限", "今年", "年度", "流年"] },
    { title: "对宫应事", keys: ["对宫", "应事", "外部", "对象", "牵动"] },
    { title: "卦线提醒", keys: ["卦", "提醒", "风险", "注意", "时机"] },
  ];
  if (!sections.length) return [];
  if (sections.length === 1) {
    const lines = splitWentianReadingParagraphs(sections[0].content || sections[0].title, 52);
  ...
  sections.forEach((section, index) => {
    if (!used.has(index) && section.content && groups.length < 3) groups.push({ title: section.title || "补充解读", content: section.content });
  });
  return groups.filter((item) => item.content || item.title).slice(0, 3);
}

function renderWentianXiaoLianTags(items) {
  return `
    <div class="wentian-mb-xiaolian-tags">
      ${(items || []).filter(Boolean).slice(0, 3).map((item) => `<i>${escapeHtml(item)}</i>`).join("")}
    </div>
  `;
}
...
function renderWentianXiaoLianReading(data, fallback, actionAttr) {
  ...
  const groupTags = [
    ...
    [selected.liunianGuaName, guaLine, "看时机"],
  ];
  return `
...
function renderWentianMobileChapter(chapter, index) {
  const results = wentianChartAiState.results || {};
  const result = chapter.module === "current_luck"
    ? getWentianLuckAiResultForSelected()
    : (chapter.module ? results[chapter.module] : null);
  ...
  if (chapter.module === "xiaoxian_liunian") {
    return `
      <article class="wentian-mb-chapter wentian-mb-xiaolian-chapter ${chapter.ready ? "is-ready" : "is-empty"}" data-wentian-report-chapter="${index}">
        ${renderWentianXiaoLianReading(result, chapter.placeholder, actionAttr)}
      </article>
    `;
  }
...
function getWentianChartAiChapters() {
  ...
  return WENTIAN_CHART_AI_CHAPTERS.map((chapter) => {
    const moduleReady = chapter.modules.some((moduleKey) => moduleKey === "current_luck" ? hasWentianLuckAiResult() : !!results[moduleKey]);
    const ready = moduleReady;
    const singleResult = chapter.modules[0] === "current_luck" ? getWentianLuckAiResultForSelected() : results[chapter.modules[0]];
    const body = chapter.modules.length === 1
        ? getWentianAiSummary(singleResult, 124)
...
function getWentianChartAiExtraParams(moduleKey, chartData = {}) {
  ...
  const selectedYear = moduleKey === "xiaoxian_liunian"
    ? getWentianXiaoLianPayload(getWentianSelectedXiaoLianYear(chartData))
    : getWentianAiCurrentYear(chartData);
...
async function decodeWentianChartAiModules(moduleKeys, options = {}) {
  ...
  Object.assign(wentianChartAiState, {
    ...
    luckAiResults: options.reset ? {} : { ...(wentianChartAiState.luckAiResults || {}) },
    luckDecadeKey: moduleKeys.includes("current_luck") ? (getWentianSelectedDecade(chartData).key || "") : wentianChartAiState.luckDecadeKey,
    xiaoLianAgeKey: moduleKeys.includes("xiaoxian_liunian") ? (getWentianSelectedXiaoLianYear(chartData).key || "") : wentianChartAiState.xiaoLianAgeKey,
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      if (task.module === "current_luck") {
        ...
        if (selectedKey) wentianChartAiState.luckAiResults[selectedKey] = taskResult;
        if (!selectedKey || selectedKey === selected.currentKey) wentianChartAiState.results.current_luck = taskResult;
      } else {
        wentianChartAiState.results[task.module] = taskResult;
      }
      success += 1;
      saveWentianChartAiState();
    } catch (error) {
...
const WENTIAN_I18N_EN_EXTRA = {
  ...
  "· 覆盖事业、情感、财富等核心场景": "· Career, love, wealth, and key timing",
  "· 结合命盘结构输出高价值行动建议": "· Turns chart patterns into action advice",
  "· 下单后自动生成，可在我的报告持续复盘": "· Saved in My Reports after purchase",
  "八字": "Bazi",
  ...
  "每日自动刷新。": "Resets daily.",
  "只按每日额度计算：免费 20次/天，付费 100次/天。": "Daily only: free 20/day, paid 100/day.",
  "免费用户 20次/天，付费用户 100次/天；每日刷新，不设月额度。": "Free 20/day, paid 100/day; resets daily with no monthly quota.",
  "开通付费版": "Open Paid Plan",
...
Object.assign(WENTIAN_I18N_EN_EXTRA, {
  ...
  "已登陆": "Signed In",
  "未建档": "No Profile",
  "先建立命盘档案": "Create a chart profile first",
  "换档案": "Switch",
  "紫微斗数": "Zi Wei",
  "排盘 · AI细读": "Chart + AI",
  ...
  "清除": "Clear",
  "改时间": "Edit Time",
  "编辑命盘时间": "Edit Birth Time",
  "出生日期时间": "Birth Date & Time",
  "保存后会重新排盘，并刷新合盘结果。": "Saving will recalculate the chart and refresh the compatibility result.",
  "保存时间": "Save Time",
  ...
  "提交占问通过后，进入全屏投币。": "Coin casting opens after approval.",
  "待投铜钱": "Coins Waiting",
  "正": "Heads",
  "反": "Tails",
  ...
  "今日剩余 20/20 · 每日额度 20次/天": "Today remaining 20/20 · Daily quota 20/day",
  ...
  "今日剩余 20/20": "Today remaining 20/20",
  "每日额度 20次/天": "Daily quota 20/day",
  "保存到本机": "Save Locally",
...
const WENTIAN_I18N_ZH_HANT_PHRASES = {
  "登录": "登入",
  "注册": "註冊",
  "账号": "帳號",
  "账户": "帳戶",
...
function translateWentianText(text, code = getWentianLanguageCode(), element = null) {
  ...
  if (lang === "zh-Hant") {
    ...
    return dict[source] || convertWentianTextToTraditional(source);
  }
  if (lang === "en") {
    if (source.includes("\n") || source.includes("\\n")) {
      return source.split(/(?:\\n|\n)+/).map((line) => translateWentianText(line, code, element)).join("\n");
...
function getWentianXuChatPayload() {
  ...
  if (context?.type === "yijing" && isWentianUuid(context.recordId)) {
    ...
    const yijingChartData = {
      ...baseChartData,
      chartRecordId: context.recordId,
      sourceChartRecordId: context.sourceChartRecordId || baseChartData.chartRecordId,
      chatMode: "yijing",
      source: "易经推命",
      yijingContext: context,
    };
    return {
      mode: "yijing",
      chartRecordId: context.recordId,
      chartData: yijingChartData,
      divinationContext: context,
    };
  }
  const chartData = getWentianChartPayload();
...
function buildWentianXuOutboundMessage(message, context) {
  ...
  if (context?.type === "yijing") {
    return [
      "【易经推命追问】",
      "请结合当前命盘和易经推命模块回答，重点围绕当前卦象、先后天/流年位置与命盘主线。",
      `当前模块：${context.tabTitle || ""}`,
      `卦象：${context.hexagramName || ""}`,
      context.liunianHeadline ? `流年：${context.liunianHeadline}` : "",
      context.original ? `古籍原文：${context.original}` : "",
      context.masterTitle ? `名师讲解标题：${context.masterTitle}` : "",
      context.detail ? `名师讲解：${context.detail}` : "",
      "回答要求：先用白话给结论，再讲依据和行动建议；不要长篇照抄原文。英文模式下只输出自然英文。",
      "",
      `我的追问：${message}`,
    ].filter(Boolean).join("\n");
  }
...
function getWentianXuModeText(mode, phase = "ready") {
  const map = {
    ...
    yijing: { connecting: "接入易经推命中…", ready: "易经推命已接入", typing: "许大师正在看这组卦…" },
...
function getWentianXuOpeningText(payload = getWentianXuChatPayload()) {
  if (isWentianEnglishMode()) {
    ...
    if (payload.mode === "yijing") return "I have this hexagram reading loaded. Ask your follow-up in English.";
  ...
  if (payload.mode === "yijing") return `这组${payload.divinationContext?.summaryLine || "易经推命"}已接入。你可以继续问成败、应期、家宅、六亲或下一步怎么做。`;
  return "我在，看命盘直接问。";
}
...
function sourceMembershipScreen() {
  ...
  return `
    ...
    ${figText("wt33-free-quota", "20次/天", 174, 314, 160, 14, "#8d8377", 800, "right")}
...
function renderWentianHepanChartCard(archive, display, label, nodeId, side = "left") {
  ...
  return `
    <article class="wentian-hepan-chart-card">
      ...
      ${chartHtml || `<div class="wentian-hepan-chart-empty">此档案暂缺完整命盘，先按四柱与宫位摘要合参：${escapeHtml(item.pillars || "资料不足")}</div>`}
    </article>
  `;
}
...
async function reviewLiuyaoQuestionViaChat(question) {
  ...
  try {
    ...
    const quota = normalizeLiuyaoQuota(getLiuyaoState().quota);
...
function renderLiuyaoCoinSummary(state, options = {}) {
  ...
  const buttonText = options.complete
    ? "卦已成"
    : (options.lockText || (tossing ? "查看落币" : `投第 ${progress + 1} 爻`));
  const desc = last
  ...
  return `
    <div class="liuyao-coin-summary ${disabled ? "is-disabled" : ""}">
      ...
      ${renderLiuyaoCoinFaceStrip(last?.coins || [3, 2, 3], last ? `第 ${progress} 爻` : "待投铜钱")}
      <button type="button" data-action="liuyao-open-caster" ${disabled ? "disabled" : ""}>${escapeHtml(buttonText)}</button>
    </div>
  `;
}
...
function renderLiuyaoCoinSummary(state, options = {}) {
  ...
  const disabled = Boolean(options.disabled && !complete);
  const tossing = liuyaoTossAnimation?.active && state.mode === "online";
  ...
  return `
    <section class="liuyao-coin-summary ${disabled ? "is-disabled" : ""} ${complete ? "is-complete" : ""}">
      <div class="liuyao-coin-summary-head">
        ...
        <b>${escapeHtml(complete ? "6/6" : `${progress}/6`)}</b>
      </div>
  ...
  return `
    <section class="liuyao-manual-card ${disabled ? "is-disabled" : ""}">
      <div class="liuyao-manual-head">
        <div>
          <span>真实铜钱录入</span>
          <strong>${escapeHtml(complete ? "六爻已录满" : LIUYAO_LINE_LABELS[currentIndex])}</strong>
        </div>
        <em>${escapeHtml(complete ? "6/6" : `${currentCoinCount}/3`)}</em>
      </div>
      <p class="liuyao-manual-copy">${escapeHtml(complete ? "六爻已满，可直接查看卦象；如需修正，可重录上一爻。" : `第 ${currentIndex + 1}/6 爻，每爻录入三枚铜钱，按正反依次确认。`)}</p>
      ${complete ? `
        <div class="liuyao-manual-done">
          <strong>卦已成</strong>
          <span>本卦、变卦和动爻都已生成。</span>
          <div class="liuyao-manual-current-actions">
            <button type="button" class="liuyao-manual-clear" data-action="liuyao-manual-clear-last" ${disabled ? "disabled" : ""}>重录上一爻</button>
            <button type="button" class="liuyao-manual-confirm" data-action="liuyao-show-result">查看解卦</button>
          </div>
        </div>
      ` : `
        <div class="liuyao-manual-current">
          ...
          <div class="liuyao-manual-status">${escapeHtml(currentCoinCount >= 3 ? `本爻已成：${currentCast?.value || ""} ${currentCast ? getLiuyaoLineType(currentCast.value).name : ""}，确认后进入下一爻。` : "抛三枚铜钱后，依次录入第 1、2、3 枚。")}</div>
          <div class="liuyao-manual-coins">
            ${[0, 1, 2].map((coinIndex) => `
              <div class="liuyao-manual-coin-pick">
                <i>第 ${coinIndex + 1} 枚</i>
...
const YANGZHAI_OPTIONS = [
  ...
  { label: "爷爷（外公）", short: "爷爷", type: "family", elderKey: "grandpa", mappedLabel: "父亲" },
  { label: "奶奶（外婆）", short: "奶奶", type: "family", elderKey: "grandma", mappedLabel: "母亲" },
...
const YANGZHAI_ELDER_ROLE_COPY = {
  grandpa: {
    familyClass: "父位类",
    introNote: "现（爷爷、外公）居此位，65岁之前按父亲位解读。",
    retirementNote: "65岁后应退位（不再担一家之主的角色），退居东北位为佳。"
  },
  grandma: {
    familyClass: "母位类",
    introNote: "现（奶奶、外婆）居此位，65岁之前按母亲位解读。",
    retirementNote: "65岁后应退位（不再担一家之主的角色），退居东北位为佳。"
  }
};
const YANGZHAI_AVATAR_IMAGES = {
  ...
  "爷爷（外公）": "../images/wentian-prototype-assets/yangzhai-avatar-fuqin.webp",
  "奶奶（外婆）": "../images/wentian-prototype-assets/yangzhai-avatar-muqin.webp"
};
...
function loadYangzhaiState() {
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      const state = {
        ...
        elderAges: shouldResetForBlankDefault
          ? { ...YANGZHAI_ELDER_AGE_DEFAULTS }
          : { ...YANGZHAI_ELDER_AGE_DEFAULTS, ...(parsed.elderAges || {}) }
      };
  ...
  return {
    ...
    pendingItems: [],
    expanded: {},
    elderAges: { ...YANGZHAI_ELDER_AGE_DEFAULTS }
  };
}
...
function saveYangzhaiState() {
  try {
    if (typeof localStorage !== "undefined") {
      ...
      yangzhaiState.elderAges = { ...YANGZHAI_ELDER_AGE_DEFAULTS, ...(yangzhaiState.elderAges || {}) };
...
function getYangzhaiMappedLabel(label) {
  return getYangzhaiOption(label).mappedLabel || label;
}

function isYangzhaiElderLabel(label) {
  return Boolean(getYangzhaiOption(label).elderKey);
}

function getYangzhaiElderAgeValue(elderKey) {
  return yangzhaiState.elderAges?.[elderKey] || YANGZHAI_ELDER_AGE_DEFAULTS[elderKey] || "65plus";
}

function getYangzhaiElderAgeLabel(elderKey) {
  return YANGZHAI_ELDER_AGE_LABELS[getYangzhaiElderAgeValue(elderKey)] || YANGZHAI_ELDER_AGE_LABELS["65plus"];
}

function getYangzhaiDisplayLabel(label, mode = "default") {
  const option = getYangzhaiOption(label);
  if (!option.elderKey) return label;
  if (mode === "pill") return `${option.short}${getYangzhaiElderAgeValue(option.elderKey) === "under65" ? "65-" : "65+"}`;
  if (mode === "selection") return `${option.label} · ${getYangzhaiElderAgeLabel(option.elderKey)}`;
  if (mode === "tooltip") return `${option.label}（${getYangzhaiElderAgeLabel(option.elderKey)}）`;
  if (mode === "short") return option.short;
  return option.label;
}

function getYangzhaiElderReadingLines(label) {
  const option = getYangzhaiOption(label);
  if (!option.elderKey) return [];
  return [YANGZHAI_ELDER_ROLE_COPY[option.elderKey]?.introNote].filter(Boolean);
}

function getYangzhaiElderClosingLines(label) {
  const option = getYangzhaiOption(label);
  if (!option.elderKey || getYangzhaiElderAgeValue(option.elderKey) === "under65") return [];
  return [YANGZHAI_ELDER_ROLE_COPY[option.elderKey]?.retirementNote].filter(Boolean);
}
...
function setYangzhaiElderAge(elderKey, ageValue) {
  if (!elderKey || !YANGZHAI_ELDER_AGE_LABELS[ageValue]) return;
  yangzhaiState.elderAges = {
    ...YANGZHAI_ELDER_AGE_DEFAULTS,
    ...(yangzhaiState.elderAges || {}),
    [elderKey]: ageValue
  };
...
function resetYangzhai() {
  ...
  yangzhaiState.elderAges = { ...YANGZHAI_ELDER_AGE_DEFAULTS };
  saveYangzhaiState();
...
function getYangzhaiHex(label, palace) {
  const upper = YANGZHAI_ROLE_GUA[getYangzhaiMappedLabel(label)];
...
function buildYangzhaiResults() {
  return YANGZHAI_PALACES
    ...
    .flatMap((palace) => getYangzhaiPlacementItems(palace.key).map((label) => {
      const lang = getWentianLanguageCode();
      const option = getYangzhaiOption(label);
      const mappedLabel = getYangzhaiMappedLabel(label);
      const displayLabel = getYangzhaiDisplayLabel(label, "default");
      const short = isYangzhaiElderLabel(label) ? getYangzhaiDisplayLabel(label, "short") : option.short;
      const readingLines = getYangzhaiElderReadingLines(label);
      ...
      const matched = mappedLabel === palace.defaultItem;
      const reading = getYangzhaiHexReading(hex);
      const originalText = formatYangzhaiReadingText(getYangzhaiReadingText(reading));
      const closingLines = getYangzhaiElderClosingLines(label);
      const fallbackText = `已安位到${palace.dir}，该组合暂无索引。`;
      const bodyText = [...readingLines, hex ? (originalText || "原文待核。") : fallbackText, ...closingLines].join("\n");
      const result = lang === "en" && hex ? {
        ...
        kind: "hex",
        title: `${displayLabel}住${palace.gua}(${palace.dir}) - ${hex ? hex.name : palace.role}`,
        desc: bodyText,
        full: hex
          ? `结构：${hex.index}；上卦=角色(${YANGZHAI_ROLE_GUA[mappedLabel]})，下卦=方位(${palace.gua})。原文摘录：${bodyText}`
          : bodyText,
        short,
        matched
      };
...
function yangzhaiRoomAvatar(id, x, y, labels, compact = false) {
  ...
  const label = items[0];
  const displayLabel = items.length > 1
    ? `${getYangzhaiDisplayLabel(label, isYangzhaiElderLabel(label) ? "short" : "default")}等${items.length}项`
    : getYangzhaiDisplayLabel(label, isYangzhaiElderLabel(label) ? "pill" : "default");
  const pillW = compact ? 72 : 72;
  const pillY = y + (compact ? 58 : 74);
  const labelSize = displayLabel.length > 6 ? 9 : (displayLabel.length > 4 ? 10 : (compact ? 10 : 11));
  return `
...
function yangzhaiCompassShell(id, compact = false, showPrompt = true) {
  const metrics = getYangzhaiGridMetrics(compact);
  return `
    ...
    ${compact || !showPrompt ? "" : `<div class="yangzhai-placement-prompt" data-node-id="${id}-placement-prompt" data-yangzhai-placement-prompt style="left:${metrics.x}px;top:${metrics.y + metrics.gridH + 5}px;width:${metrics.gridW}px;">开指南针，对准正北/正东/正南/正西后安位。</div>`}
  `;
}
...
function sourceYangzhaiResultScreen() {
  ...
  return `
    ...
    ${!results.length ? `
      ...
      ${figText("yz44-empty-copy", "添加家人、长辈、厨房、厕所或客厅后再分析。", 0, YANGZHAI_RESULT_START_Y + 70, 390, 12, "#6e6254", 600, "center")}
    ` : ""}
...
function isOfficeLayoutRoute(route = state.route) {
  return ["screen-50", "screen-51", "screen-52"].includes(route);
}
...
function updateOfficeLayoutField(field, value) {
...
function sourceOfficeLayoutHomeScreen() {
  return sourceOfficeLayoutFrame("office50", "办公室布局", `
    <section class="office-layout-panel hero">
      ...
      <p class="office-layout-copy">根据办公室大门朝向和老板位置，快速生成一版布局摘要、调整建议和长文解读。</p>
...
function sourceOfficeLayoutGuideScreen() {
  return sourceOfficeLayoutFrame("office51", "使用说明", `
    ...
    <section class="office-layout-guide-grid">
      ...
      <article class="office-layout-guide-step"><i>03</i><div><strong>选外卦和内卦，再查询</strong><p>系统会给出摘要卡、核心结论、布局建议和长文解读。</p></div></article>
    </section>
    <section class="office-layout-panel">
      <h3>不会确定方位？</h3>
      <p class="office-layout-copy">1）先找平面图上的北；2）再看大门在哪一边；3）最后看老板位靠哪一宫。没有十足把握时，先用指南针复核后再选。</p>
      <div class="office-layout-buttons">
...
function renderWentianPolishedScreen(screen) {
  ...
  if (false && no === 21) {
    return `
      ...
      ${figText("wt21-desc", "免费用户 20次/天，付费用户 100次/天；每日刷新，不设月额度。", 70, 450, 250, 13, "#756d63", 500, "center", "line-height:1.5;")}
  ...
  if (no === 29) {
    ...
    return `
      ...
      ${figText("wt29-free-quota", "20次/天", 220, 336, 100, 15, "#8d8377", 900, "right")}
  ...
  if (no === 33) {
    return `
      ...
      ${figText("wt33-card-sub", "20次/天，按日刷新", 52, 170, 160, 13, "#c7bda8", 600)}
...
function convertedSpecial(screen) {
  ...
  if (screen.recharge) {
    return `
      ...
      ${[["免费 20次/天", 42, 220], ["付费 100次/天", 42, 332]].map(([label, x, y], index) => `
...
function sourceDashboardHomeScreen() {
  {
    ...
    return `
      ...
      ${figBox("source-1-hero-secondary", 158, 226, 92, 34, "", "border-radius:17px;background:rgba(255,250,243,.16);border:1px solid rgba(255,250,243,.36);")}
      ${figText("source-1-hero-secondary-text", "去问他", 158, 236, 92, 12, "#fffaf3", 900, "center")}
      ${figButton("source-1-hero-secondary-hit", 158, 226, 92, 34, 'data-route="screen-5"', "", "z-index:35;")}
...
function renderWentianDatasetAttrs(dataset = {}) {
  return Object.entries(dataset || {}).map(([key, value]) => {
    if (value === null || value === undefined || value === "") return "";
    const dataKey = String(key).replace(/([A-Z])/g, "-$1").toLowerCase();
    return ` data-${escapeHtml(dataKey)}="${escapeHtml(String(value))}"`;
  }).join("");
}
...
function normalizeWentianClassicChartPalace(palace) {
  if (!palace) return null;
  const branch = palace.earthlyBranch || palace.branch || "";
  if (!branch) return null;
  return {
    ...
    majorStars: (palace.majorStars || []).map(normalizeWentianStar).filter(Boolean),
    minorStars: (palace.minorStars || []).map(normalizeWentianStar).filter(Boolean),
    decadal: palace.decadal || (palace.range ? { range: palace.range } : null),
    changsheng12: palace.changsheng12 || "",
    boshi12: palace.boshi12 || "",
    isBodyPalace: !!palace.isBodyPalace,
  };
...
function getWentianClassicChartRenderState(saved) {
  const source = saved || {};
  const chartData = source.chartData || {};
  const chart = source.chart || {};
  const chartPalaces = Array.isArray(chart.palaces) ? chart.palaces : [];
  const summaryPalaces = Array.isArray(chartData.palacesSummary) ? chartData.palacesSummary : [];
  const palaces = (chartPalaces.length ? chartPalaces : summaryPalaces)
    .map(normalizeWentianClassicChartPalace)
    .filter(Boolean);
  if (!palaces.length) return source;
  const lifePalace = chartData.lifePalace || palaces.find((palace) => String(palace.name || "").replace(/宫$/, "") === "命") || {};
  const bodyPalace = chartData.bodyPalaceDetail || palaces.find((palace) => palace.isBodyPalace) || {};
  const dateText = chartData.birthDate || chartData.solarTime || source.form?.datetime || "";
  return {
    ...source,
    chartData,
    chart: {
      ...chart,
      palaces,
      solarDate: chart.solarDate || String(dateText).slice(0, 10),
      lunarDate: chart.lunarDate || chart.chineseDate || chartData.lunarDate || chartData.chineseDate || "",
      chineseDate: chart.chineseDate || chartData.chineseDate || "",
      fiveElementsClass: chart.fiveElementsClass || chartData.fiveElementsClass || "",
      zodiac: chart.zodiac || chartData.zodiac || "",
      earthlyBranchOfSoulPalace: chart.earthlyBranchOfSoulPalace || lifePalace.branch || lifePalace.earthlyBranch || "",
      earthlyBranchOfBodyPalace: chart.earthlyBranchOfBodyPalace || bodyPalace.branch || bodyPalace.earthlyBranch || "",
    },
  };
}

function renderWentianClassicChart(saved, options = {}) {
  const source = getWentianClassicChartRenderState(saved);
  const chart = source.chart || {};
      ...
      <div class="fc-card">
        <div class="fc-grid-wrap">
          <div class="fc-grid">
            ...
            ${renderWentianClassicCenter(chart, chartData, form, centerOptions, centerSource)}
          </div>
        </div>
      </div>
    </div>`;
...
function getWentianMobileParamContext(saved) {
  ...
  const form = saved?.form || {};
  const formDate = form.datetime ? new Date(form.datetime) : null;
  const savedChartData = saved?.chartData || {};
  const builtChartData = chart ? buildWentianChartPayload(chart, {
    gender: form.gender || savedChartData.gender || "male",
    date: formDate && !Number.isNaN(formDate.getTime()) ? formDate : null,
    timeIndex: Number(savedChartData.timeIndex),
    city: form.city || savedChartData.city || "",
  }) : getWentianChartPayload();
  const chartData = savedChartData.palacesSummary?.length
    ? savedChartData
    : { ...builtChartData, ...savedChartData, palacesSummary: builtChartData.palacesSummary || savedChartData.palacesSummary || [] };
  const palaces = chartData?.palacesSummary?.length
    ? chartData.palacesSummary
    : (chart?.palaces || []).map(normalizeWentianPalace).filter(Boolean);
  const findPalace = (name) => {
    const target = String(name || "").replace(/宫$/, "");
    return palaces.find((palace) => String(palace?.name || "").replace(/宫$/, "") === target) || null;
  };
  const findPalaceByBranch = (branch) => palaces.find((palace) => palace?.branch === branch || palace?.earthlyBranch === branch) || null;
  const lifeBranch = chart?.earthlyBranchOfSoulPalace || chartData?.lifePalace?.branch || findPalace("命")?.branch || "";
  const bodyBranch = chart?.earthlyBranchOfBodyPalace || chartData?.bodyPalaceDetail?.branch || "";
  return { chart, chartData: { ...chartData, palacesSummary: palaces }, form, palaces, findPalace, findPalaceByBranch, lifeBranch, bodyBranch };
}

function formatWentianParamValue(value, fallback = "—") {
  const text = Array.isArray(value) ? value.filter(Boolean).join(" · ") : value;
  return String(text == null || text === "" ? fallback : text);
}

function formatWentianParamStars(stars, limit = 8) {
  const list = normalizeWentianAiStarList(stars)
    .slice(0, limit)
    .map((star) => `${star.name || ""}${star.brightness || ""}${star.mutagen || ""}`)
    .filter(Boolean);
  return list.length ? list.join("、") : "空宫";
}

function renderWentianParamPair(label, value) {
  return `<div><span>${escapeHtml(label)}</span><b>${escapeHtml(formatWentianParamValue(value))}</b></div>`;
}

function renderWentianParamPillar(label, value) {
  return `<div><span>${escapeHtml(label)}</span><b>${escapeHtml(formatWentianParamValue(value))}</b></div>`;
}
...
function renderWentianMobileChartParams(saved) {
  const ctx = getWentianMobileParamContext(saved);
  const chartData = ctx.chartData || {};
  const chart = ctx.chart || {};
  const form = ctx.form || {};
  const sizhu = chartData.sizhu || extractWentianPillars(chart);
  const genderText = form.gender === "female" || chartData.gender === "female" ? "女" : "男";
  const timeName = WENTIAN_SHICHEN[Number.isFinite(Number(chartData.timeIndex)) ? Number(chartData.timeIndex) : -1] || "";
  const lifePalace = ctx.findPalaceByBranch(ctx.lifeBranch) || chartData.lifePalace || ctx.findPalace("命");
  ...
  const palaceOrder = ["命宫", "兄弟宫", "夫妻宫", "子女宫", "财帛宫", "疾厄宫", "迁移宫", "仆役宫", "官禄宫", "田宅宫", "福德宫", "父母宫"];
  const orderedPalaces = palaceOrder
    .map((name) => ctx.findPalace(name.replace(/宫$/, "")))
    .filter(Boolean);
  const remainingPalaces = ctx.palaces.filter((palace) => !orderedPalaces.includes(palace));
  return `
    <section class="wentian-chart-param-panel" data-node-id="source-27-param-panel">
      ...
      <div class="wentian-param-pillars">
        ...
        ${renderWentianParamPillar("月柱", sizhu.month || `${sizhu.monthStem || ""}${sizhu.monthBranch || ""}`)}
...
function getWentianYijingContext(saved, selectedXiaoLian = null) {
  const ctx = getWentianMobileParamContext(saved);
  const chartData = ctx.chartData || {};
  const form = ctx.form || {};
  ...
  const selectedXiao = selectedXiaoLian || getWentianSelectedXiaoLianYear(chartData);
...
function makeWentianYijingXuContext(saved = getWentianSavedChart()) {
  ...
  return {
    type: "yijing",
    recordId: makeWentianUuid(),
    sourceChartRecordId: chartData.chartRecordId,
    title: "易经推命",
    summaryLine: `${meta.title}${result.name ? ` · ${result.name}` : ""}`,
    tabKey: activeKey,
    tabTitle: meta.title,
    hexagramName: result.name || "",
    hexagramNo: result.no || result.num || "",
    liunianHeadline,
    original: reading.original || "",
    masterTitle: reading.masterTitle || "",
    summary: reading.summary || "",
    detail: reading.detail || reading.summary || "",
  };
}

function openWentianYijingXuChat() {
  const context = makeWentianYijingXuContext();
  if (!context) {
...
function syncWentianPhoneBackgroundHeights(phone, nextHeight) {
  ...
  for (const node of phone.querySelectorAll(".fig-box")) {
    ...
    if (left !== 0 || top !== 0 || Math.round(width) < WENTIAN_PHONE_WIDTH) continue;
    if (height < currentHeight - 1) continue;
    node.style.height = `${nextHeight}px`;
  }
}

function compactWentianBottomNavGap(phone = view?.querySelector?.(".figma-phone")) {
  if (!phone) return false;
  const navSelector = '[data-node-id^="source-bottom-"],[data-node-id^="converted-bottom-"],[data-node-id^="bottom-"]';
  const navNodes = Array.from(phone.querySelectorAll(navSelector));
  ...
  if (!Number.isFinite(navTop) || !Number.isFinite(navBottom)) return false;
  const phoneHeight = parseFloat(phone.style.height) || phone.offsetHeight || WENTIAN_PHONE_HEIGHT;
  if (phoneHeight <= navBottom + 4) return false;

  let maxBottom = 0;
  const contentNodes = phone.querySelectorAll('[data-node-id], input, select, textarea, .wentian-chart-content-stack, .liuyao-phone-screen, .wentian-hepan-result-panel, .wentian-native-mingpan');
  ...
  const nextHeight = Math.max(Math.ceil(navBottom), Math.ceil(maxBottom));
  if (phoneHeight - nextHeight < 8) return false;
  phone.style.height = `${nextHeight}px`;
  syncWentianPhoneBackgroundHeights(phone, nextHeight);
  return true;
}
...
function renderMine() {
  return figPhone("17:97", "Hi-Fi 03 我的", `
    ...
    ${figText("17:109", "免费20次/天，付费100次/天", 118, 229, 170, 17, "#75756e")}
...
function renderRecharge() {
  return figPhone("17:49", "Hi-Fi 02 阅天套餐", `
    ...
    ${figText("17:58", "20次/天", 122, 214, 120, 36, "#ba8f38", 700)}
    ...
    ${figText("17:63", "20次/天", 248, 416, 84, 18, "#ba8f38", 700, "right")}
...
document.addEventListener("click", (event) => {
  ...
  if (earlyAction === "yangzhai-elder-age") {
    setYangzhaiElderAge(earlyActionTarget.dataset.elderKey || "", earlyActionTarget.dataset.elderAge || "");
    return;
  }
  ...
  if (action === "wentian-hepan-xiaolian-pick") {
    const target = event.target.closest("[data-xiaolian-age-key]");
    const side = target?.dataset.hepanSide === "right" ? "right" : "left";
    wentianHepanXiaoLianAge[side] = target?.dataset.xiaolianAgeKey || "";
    navigatePreservingScroll("screen-49", false);
    return;
  }
  ...
  if (action === "wentian-yijing-ask-xu") {
    openWentianYijingXuChat();
    return;
  }
