...
- 后端渠道明确返回可用时，电脑和手机都按二维码方式处理。
- 前端在拿不到渠道状态时默认不启用支付宝；遇到签约/接口权限不足会关闭支付宝并切换到微信或 PayPal。
...
- `c981106`：支付宝二维码权限不足时保留错误码、禁用支付宝并切换到可用渠道
- `a69b7fe`：拿不到后端渠道状态时默认关闭支付宝，避免进入必然失败的结账
...
本交接整理期间，`js/yl.js` 曾出现并行改动，随后已经独立提交为 `c981106 fix: recover from unavailable Alipay QR`。该提交做了三件事：

- `apiFetch()` 保留后端错误码
- 识别支付宝 `ALIPAY_PERMISSION_REQUIRED` / 接口权限不足
- 支付宝无权限时禁用该渠道并切到可用的微信或 PayPal

这份支付宝兜底代码没有在本交接窗口内重新做浏览器回归；继续支付工作时，应先验证支付宝权限不足、自动切换渠道和正常支付不受影响。
