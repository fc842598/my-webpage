...
- 前端分支：`master`
- 前端统一结账功能基线：`63bb11b feat: embed login in membership checkout`
...
前端当前不是完全干净状态。`js/yl.js` 在本交接整理期间出现了已有的未提交改动，不是交接文档产生的，新窗口不要回滚或和本文档一起提交。该差异目前做了三件事：

- `apiFetch()` 保留后端错误码
- 识别支付宝 `ALIPAY_PERMISSION_REQUIRED` / 接口权限不足
- 支付宝无权限时禁用该渠道并切到可用的微信或 PayPal

这份 `js/yl.js` 改动尚未完成本轮浏览器验证和提交，继续处理前必须先复查差异、确认是否保留。
