...
每日发布10-30篇中文文章及自然英文改写版：保底10篇、常规20篇、质量和需求充足时最多30篇。每篇都必须独立、有用户需求且有源文支撑；不合格稿必须换题重写，不能用产品帮助、近义标题或固定模板凑数量。

流量优先时，关键词不是标题词组，而是“真实查询 + 搜索原因 + 用户要完成的判断”。已有查询匹配旧页时优先升级旧URL；只有搜索任务和答案路径都明显不同才新建页面。每天维持至少3天候选库存，单篇失败时用库存替换，不能让一篇问题稿拖停整天。
...
5. 质量闸门：先确定当天最终数量 `N`（10-30），运行 `npm run articles:quality-gate -- --seed ... --docx ... --date ... --expected-count N`。失败稿必须剔除或替换，再把最终seed重排为1-N并重新全量验证；正式队列中的N篇必须全部通过。
...
8. 发布与复查：队列校验、单篇释放器和底层发布器都必须使用当前质量规则重新读取指定DOCX并全量验证当天N篇，不能只信准备阶段的质量报告或五星哈希；通过后中英文配对使用同一分钟，同步索引、Feed、Sitemap、JSON-LD 与 hreflang，推送后抽查线上页面。
...
- 初稿通过自动门后，必须由紫微逻辑、中文真人读感、SEO搜索意图、英文自然度、用户价值五个独立评论席逐篇审查当天N篇；任何一席出现一篇 `FAIL`，该稿不得进入最终批次。
- 返工或替换后必须保留第一轮报告，并由五席重新全量复审最终N篇，不能只复查失败句。只有五份最终报告全部为 `N PASS / 0 FAIL`，才允许生成源稿、队列和系统定时任务。
...
合格结构样例见 `scripts/fixtures/valid-daily-ziwei-seed.mjs`；单篇结构测试时质量门可显式传入 `--expected-count 1`，生成器还必须同时传入 `--test-mode true`。正式生产固定为10-30篇，低于10篇只能测试，禁止测试模式上线。
...
- 当日有真实信号时，最低数据锚点数为 `min(8, 可核验信号数)`；只有达到扩展门槛的 `winners` 页面能支撑相邻新题，高展现低点击页与低样本观察页只能优化原页；同一搜索词、表现页面或编辑场景最多支撑2篇，批内用户搜索问法不得重复
...
质量报告的 `evidence.binding` 会保存每条观点、每个例子的最佳证据范围、覆盖率、实际使用范围数和当前阈值。修改这套规则后必须运行 `npm run articles:evidence-binding:test`，并对当天正式批次重新生成质量报告、源稿和发布队列。
